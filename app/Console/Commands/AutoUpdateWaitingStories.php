...
class AutoUpdateWaitingStories extends Command
{
    ...
    public function handle()
    {
        ...
        // Find all stories in Waiting status
        $waitingStories = Story::where('status', StoryStatus::Waiting->value)->get();
        
        if ($waitingStories->isEmpty()) {
        ...
        foreach ($waitingStories as $story) {
            try {
                // Validate story has an ID
                if (!$story || !$story->id) {
                    $this->warn("Skipping story without valid ID.");
                ...
                if (!$waitingEntryDate) {
                    $this->warn("Story ID {$story->id}: Could not determine when it entered Waiting status. Skipping.");
                ...
                if (!$previousStatus) {
                    ...
                    $this->info("Story ID {$story->id}: No previous status found in logs, using New as fallback.");
                ...
                $this->info("Story ID {$story->id}: Restored from Waiting to {$targetStatus} (was in Waiting for {$daysInWaiting} days).");
        ...
        $this->info("Command completed. Success: {$successCount}, Skipped: {$skippedCount}, Errors: {$errorCount}.");
        Log::info("orchestrator:autoupdate-waiting completed. Success: {$successCount}, Skipped: {$skippedCount}, Errors: {$errorCount}.");
    }
