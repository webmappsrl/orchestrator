...
## Cosa cambia
Orchestrator espone una nuova view "Hetzner Monitoring" che aggrega in tempo reale (con cache Redis) i dati di tutti i progetti Hetzner Cloud aziendali, mostrando per ogni progetto: server con status e costo, Floating IP, Volumes, Load Balancers e Snapshots. La view include un export, un pulsante di refresh manuale della cache e **note persistenti per risorsa** (server, IP, volume, LB, snapshot) salvate in PostgreSQL e visibili dopo ogni reload.
...
- [ ] Calcolare costo mensile per risorsa e totale per progetto (prezzi da API, con nota esplicita che sono prezzi di listino, escluse eventuali promozioni o crediti)
...
- [ ] Export con tutti i dati visibili nella view
...
## Rischi
- **Costo stimato ≠ fattura reale**: i prezzi esposti sono i prezzi di listino dell'API Cloud. Il billing reale (con sconti, crediti, trial) è su un endpoint separato.
- **Token in ENV**: aggiungere un nuovo progetto richiede aggiornare le variabili d'ambiente sul server e riavviare i processi PHP (nessun deploy di codice). Non è gestibile da UI Nova — accettato per questa iterazione; `config/hetzner.php` è predisposto per futura migrazione a DB.
