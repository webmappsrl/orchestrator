...
- **Collisione di classe CSS `.tab-item` tra `Tab::group` di Nova e Toast UI Editor — risolta**: il selettore globale di Nova `.tab-group .tab-item` (padding, flex, min-width, overflow, ecc., pensato per i tab veri) si applica per errore a **due** sotto-componenti distinti di Toast UI Editor che riusano per coincidenza la stessa classe generica `tab-item` (mai in conflitto prima, perché questo campo non era mai stato dentro un `.tab-group`): i pulsanti "Markdown"/"WYSIWYG" (`.toastui-editor-mode-switch .tab-item`) e i pulsanti "Scrivere"/"Anteprima" (`.toastui-editor-md-tab-container .tab-item`). Segnalato dall'utente con screenshot degli strumenti sviluppatore (in due passaggi separati — il secondo componente non era stato notato al primo giro). Fix: due regole CSS mirate in `public/nova-custom.css`, scoped rispettivamente a ciascun sotto-componente (specificità maggiore del selettore di Nova), che ripristinano esattamente lo stile originale del componente (letto da `vendor/datomatic/nova-markdown-tui/dist/css/entry.css`) senza toccare i tab reali di Nova. Verificato che non ci sono regressioni sui 4 tab veri né altrove nell'app.
- **Overflow della toolbar del campo Note (Toast UI Editor / `datomatic-nova-markdown-tui`) peggiorato dal nesting in `Tab::group`**: la toolbar di questo componente non è responsive (richiede sempre ~993-1023px) — difetto **preesistente**, già presente in produzione oggi (verificato con test A/B: overflow di 109px anche nel codice pre-oc:8407, probabilmente invisibile su schermi larghi). Il `Tab::group` restringe la colonna del campo di ~127px (884px→757px), portando l'overflow a 266px, chiaramente visibile. Root cause confermata: spostando temporaneamente il campo fuori da `Tab::group` (in un Panel separato, mirror di `Customer.php`) la colonna torna a 884px e l'overflow torna a essere quello preesistente di produzione. **Deciso dallo sviluppatore**: lasciare il campo dentro il tab Main (rispettando l'ordine campi esatto richiesto dal ticket), accettando l'overflow più visibile. Il difetto preesistente di 109px resta comunque fuori scope (bug generale del componente, usato in tutto il progetto — es. Story, Customer — non specifico a Quote): da aprire come ticket dedicato.
