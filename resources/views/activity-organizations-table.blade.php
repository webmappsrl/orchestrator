...
<head>
    <meta charset="UTF-8">
    <title>Activity Organizations Table</title>
    <style>
        body {
...
<body>
    ...
    <h1>Attività per Organization: {{ $startDate->format('d/m/Y') }} - {{ $endDate->format('d/m/Y') }}</h1>
    ...
    <table>
        ...
        <tbody>
            @php
                // Sort organizations by name alphabetically
                usort($organizationStats, function ($a, $b) {
                    return strcasecmp($a['name'], $b['name']);
                });
            @endphp
            @forelse($organizationStats as $organizationStat)
                @php
                    $totalHours = floor($organizationStat['total_minutes'] / 60);
                    $totalMinutes = $organizationStat['total_minutes'] % 60;
                    $timeDisplay = $totalHours > 0 ? "{$totalHours}h {$totalMinutes}m" : "{$totalMinutes}m";
                    
                    // Calculate average time per ticket
                    $avgMinutes = $organizationStat['ticket_count'] > 0 ? round($organizationStat['total_minutes'] / $organizationStat['ticket_count']) : 0;
                    ...
                    $minMinutes = !empty($organizationStat['elapsed_minutes']) ? min($organizationStat['elapsed_minutes']) : 0;
                    $maxMinutes = !empty($organizationStat['elapsed_minutes']) ? max($organizationStat['elapsed_minutes']) : 0;
                ...
                <tr class="organization-row">
                    <td>
                        <div class="text-gray-900 font-medium">
                            {{ $organizationStat['name'] }}
                        </div>
                    </td>
                    <td>
                        <div class="text-gray-700 font-semibold">
                            {{ $organizationStat['ticket_count'] }}
                        </div>
                    </td>
                    <td>
