...
<head>
    <meta charset="UTF-8">
    <title>Selettore Activity</title>
    <style>
        body {
...
<body>
    <div class="activity-selector-container">
        <div class="selector-title">📊 Activity Dashboard</div>
        
        <form method="POST" action="/set-activity-filters" class="selector-wrapper">
            @csrf
            <div class="selector-group">
                <label class="selector-label" for="developer-select">Utente:</label>
                <select name="user_id" id="developer-select" class="developer-select">
                    <option value="" {{ !session('activity_selected_user_id') ? 'selected' : '' }}>
                        {{ $currentUser->name }} (La mia attività)
                    </option>
                    @foreach($developers as $developer)
                        @if($developer->id !== $currentUser->id)
                            <option value="{{ $developer->id }}" 
                                {{ session('activity_selected_user_id') == $developer->id ? 'selected' : '' }}>
                                {{ $developer->name }}
                            </option>
                        @endif
                    @endforeach
