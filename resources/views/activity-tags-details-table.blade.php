...
<head>
    <meta charset="UTF-8">
    <title>Activity Tags Details Table</title>
    <style>
        body {
...
<body>
    ...
    <h1>
        @if($selectedTag)
            Ticket per Tag: {{ $selectedTag->name }} ({{ $startDate->format('d/m/Y') }} - {{ $endDate->format('d/m/Y') }})
        @else
            Seleziona un tag per visualizzare i ticket
        @endif
    </h1>

    @if($selectedTag && isset($totalTickets))
        <div class="summary">
    ...
    @if($selectedTag && $tickets->count() > 0)
        <table>
            <colgroup>
                <col style="width: 5%;">
                <col style="width: 25%;">
    ...
    @elseif($selectedTag && $tickets->count() == 0)
        <p>Nessun ticket trovato per il tag selezionato nel periodo indicato.</p>
    @else
        <p>Seleziona un tag per visualizzare i ticket.</p>
    @endif
