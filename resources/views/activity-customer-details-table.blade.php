...
<head>
    <meta charset="UTF-8">
    <title>Activity Customer Details Table</title>
    <style>
        body {
...
<body>
    ...
    <h1>
        @if($selectedCustomer)
            Ticket per Customer: {{ $selectedCustomer->name }} ({{ $startDate->format('d/m/Y') }} - {{ $endDate->format('d/m/Y') }})
        @else
            Seleziona un customer per visualizzare i ticket
        @endif
    </h1>

    @if($selectedCustomer && isset($totalTickets))
        <div class="summary">
    ...
    @if($selectedCustomer && $tickets->count() > 0)
        <table>
            <colgroup>
                <col style="width: 5%;">
                <col style="width: 25%;">
    ...
    @elseif($selectedCustomer && $tickets->count() == 0)
        <p>Nessun ticket trovato per il customer selezionato nel periodo indicato.</p>
    @else
        <p>Seleziona un customer per visualizzare i ticket.</p>
    @endif
