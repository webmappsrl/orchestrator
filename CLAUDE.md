...
- **Scramble non infra tutti i query param da analisi statica**: riconosce solo chiamate `$request->method('key')` con metodo mappato (`integer`/`float`/`boolean`/`enum`/`query`/`string`/`str`/`input`/`get`/`post`) in posizione AST diretta (assegnazione, argomento, elemento array) — non dentro un confronto (`===`). `sort` (letto solo in `if ($request->get('sort') === '-created_at')`) e `page` (letto solo via `$request->filled('page')`, metodo non mappato, e comunque consumato internamente da `paginate()`) restavano quindi assenti da `/docs/api`; `status[]` veniva documentato solo come `string` perché Scramble vede la prima occorrenza (`$request->input('status')`) e non sa che accetta anche un array. Fix: attributi PHP `#[QueryParameter(...)]` di `Dedoc\Scramble\Attributes` direttamente sul metodo `Api\QuoteController::index()` — letti via reflection, bypassano l'inferenza. Usare questo pattern per qualsiasi futuro query param letto in modo "non standard" (dentro condizionali, o consumato da helper Laravel come `paginate()`).
