...
| Riordino colonne e metriche lista Quotes | oc:8404 | `app/Nova/Quote.php`, `tests/Feature/QuoteNovaResourceTest.php` | Vista globale Nova Quotes: colonne riordinate (Cliente, Titolo, Stato, Proprietario, Scadenza, Totale, PDF), Titolo solo lingua attiva (campo index-only con accessor Spatie, `NovaTabTranslatable` invariato sul form), Prodotti/Recurring nascosti dall'index, nuova colonna Scadenza (due_date del task todo più vicino nel tempo, scaduto o futuro — nessun filtro "solo futuro"), eager loading anti-N+1 in `indexQuery()`, metric-card `cards()` a `1/2`+`1/2` |
...
| Riordino colonne e filtro assegnatario Task | oc:8402 | `app/Nova/Task.php`, `app/Nova/Filters/TaskAssigneeFilter.php`, `tests/Feature/TaskNovaResourceTest.php` | Vista globale Nova Tasks: colonne riordinate (Cliente, Preventivo, Task, Scadenza, Stato, Completato, Assegnatario), Scadenza solo data (`Date::make`), nuova colonna+filtro Assegnatario (derivato da `quote->user`, opzioni = utenti con almeno un task); `indexQuery()` bypassa `forUser()` solo per Admin/Manager, tutti gli altri ruoli restano scoped come prima; sub-panel Task dentro Quote resta invariato (layout originale con `DateTime::make`, nessun Assegnatario) |

## Decisioni architetturali

### Riordino colonne e metriche lista Quotes (oc:8404)
- **`NovaTabTranslatable` non supporta risoluzione a singola lingua per-request**: genera N campi Nova statici indipendenti (uno per locale), nessuna opzione nativa per "mostra solo la lingua attiva in index" — verificato nel sorgente del package. Fix: campo Nova separato `onlyOnIndex()` che riusa l'accessor Spatie `$model->title` (locale-aware, con fallback), lasciando `NovaTabTranslatable` invariato (solo `hideFromIndex()` aggiunto) per non toccare il form multilingua. Attenzione: coesistono ora tre punti di risoluzione del titolo (accessor Spatie in `Quote::title()`, field form `NovaTabTranslatable`, field index-only) — possono divergere silenziosamente su edge case futuri.
- **Colonna Scadenza senza vincolo "solo futuro"**: l'overview ereditata da una sessione precedente specificava "due_date più vicina nel futuro tra i task todo", ma le parole esatte del cliente in `customer_request` non lo richiedevano. Con `due_date` non nullable a DB, quel vincolo avrebbe fatto sparire (mostrando `—`) i task todo scaduti — il caso più urgente — dalla colonna. Rimosso in Fase: challenge: ora mostra sempre il task todo con `due_date` più vicina, scaduta o futura.
- **Eager loading filtrato in `indexQuery()`** (`->with(['tasks' => fn ($q) => $q->where('status','todo')->orderBy('due_date')])`): necessario per evitare N+1 sulla nuova colonna Scadenza — Admin/Manager vedono l'intera lista Quotes non paginata per riga singola nel query pattern precedente. Indice DB esistente `(due_date, status)` su `tasks` non ottimale per questo pattern (`quote_id` + `status` + `MIN(due_date)`); lasciato come debito noto, basso volume dati.
...
- **`public/nova-custom.css` è l'UNICO file CSS custom realmente caricato da Nova** (`Nova::style('nova-custom', public_path('/nova-custom.css'))` in `NovaServiceProvider::boot()`). Esiste un secondo file quasi omonimo `public/css/nova-custom.css`, tracciato in git ma **mai referenziato da nessuna parte** — modificarlo non ha alcun effetto (l'endpoint `/nova-api/styles/nova-custom` continua a servire il contenuto del file corretto). Verificato nel modo peggiore: un primo tentativo di fix ha editato il file sbagliato, scoperto solo controllando via `fetch()` che l'endpoint serviva ancora contenuto stantio nonostante `cache:clear`.
- **`Tab::group` di Nova stila globalmente qualsiasi elemento con classe `.tab-item` dentro un `.tab-group`**, non solo i propri tab. Il componente `datomatic/nova-markdown-tui` (Toast UI Editor) usa la stessa classe generica `.tab-item` in **due** sotto-componenti distinti — pulsanti "Markdown"/"WYSIWYG" (`.toastui-editor-mode-switch`) e "Scrivere"/"Anteprima" (`.toastui-editor-md-tab-container`) — innocuo finché il campo non è mai stato nidificato in un `Tab::group` (mai accaduto prima di questo ticket), ma ora che lo è, la regola di Nova (`padding: 1rem`, `flex`, `min-width`, `overflow: hidden`, ...) sovrascrive lo stile originale di entrambi. Fix: due regole scoped più specifiche in `public/nova-custom.css`, che ripristinano i valori originali letti da `vendor/datomatic/nova-markdown-tui/dist/css/entry.css`. Qualsiasi futuro campo `MarkdownTui` nidificato in un `Tab::group` altrove nel progetto beneficia automaticamente dello stesso fix (non è specifico a Quote).
- **Overflow della toolbar principale del campo Note (icone tabella/immagine/link/code) è un difetto preesistente del componente `datomatic-nova-markdown-tui`** (la sua toolbar non è responsive, richiede sempre ~993-1023px), non introdotto da questo ticket — verificato con test A/B (`git show HEAD~1:app/Nova/Quote.php`) che l'overflow esisteva già in produzione (109px, probabilmente invisibile su schermi larghi), solo reso più visibile (266px) dal restringimento di colonna imposto da `Tab::group` (~127px in meno). Deciso di non correggerlo in questo ciclo (accettato, mantiene l'ordine campi esatto richiesto dal ticket) — da aprire come ticket dedicato, bug generale del componente usato in tutto il progetto (Story, Customer, ecc.), non specifico a Quote.
...
### Riordino colonne e filtro assegnatario Task (oc:8402)
- **`fields()`/`filters()` scoped per `viaResource === 'quotes'`, non solo `indexQuery()`**: Nova richiama lo stesso `fields()`/`filters()` sia per la vista globale Tasks sia per il sub-panel Task dentro il dettaglio Quote (`Quote.php` usa `HasMany::make(..., Task::class)`). Il primo tentativo aveva scoped solo `indexQuery()`, facendo trapelare il riordino colonne e la nuova colonna/filtro Assegnatario anche nel sub-panel — trovato da `wm-review-ticket` come bloccante prima del commit. Estratti 3 metodi privati (`statusBadgeField()`, `completedField()`, `notesField()`) per i campi condivisi tra i due branch, evitando duplicazione e drift futuro.
- **`indexQuery()` bypassa `forUser()` solo per Admin/Manager, non per tutti**: la vista globale Tasks è visibile nel menu Nova solo ad Admin/Manager (`canSee` in `NovaServiceProvider.php`); rimuovere `forUser()` per tutti i ruoli avrebbe esposto l'intero backlog task (incluse note interne) anche a Customer/Editor che hanno accesso Nova generico. Developer, Editor, Customer restano scoped a `forUser()` esattamente come prima di questo ticket.
- **Nota**: `app/Policies/TaskPolicy.php` (introdotta da oc:8403, mergiata in `develop` durante lo sviluppo di questo ticket) già blocca con 403 chiunque non sia Admin/Manager/Developer, prima che Nova arrivi a `indexQuery()` — è il vero argine per Editor/Customer, non `forUser()`. Il testo di `docs/features/8402-.../overview.md` non è stato aggiornato per riflettere questo (scritto prima che oc:8403 fosse mergiata) — vedi `notes.md` della feature per il dettaglio.
- **Assegnatario = owner della Quote, non un campo persistito sul Task**: riusa l'accessor `Task::assignee` (già esistente da oc:8327), nessuna nuova colonna DB. Ogni task della stessa quote condivide necessariamente lo stesso assegnatario — limite noto, accettato.
